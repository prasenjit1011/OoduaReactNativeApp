...
import { ImageBackground, StyleSheet, Text, Button, Image, TextInput, View, Alert, Pressable, TouchableOpacity, ActivityIndicator, Keyboard } from "react-native";
...
const Screen = ({ navigation }) => {
	...
	const [username, onChangeUsername] = React.useState("test@gmail.com");
	const [pwd, onChangePwd] = React.useState("1234567890");
	...
	const doSignup = () => {
		...
		fetch('https://ooduacargo.com/booking/api/login.php',{
			method: 'POST',
			body: formBody,
			headers: {
				//Header Defination
		...
		.then((responseJson) => {
			...
			if(responseJson.status){
				...
				setErrortext('Credentials not match.');
				return;
			}
